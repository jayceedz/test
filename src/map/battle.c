...
#define ATK_ADD2( a , b ) { wd.damage+= a; if (flag.lh) wd.damage2+= b; }
			...
			switch( skill_num )
			{
				...
				case MO_EXTREMITYFIST:
					{	//Overflow check. [Skotlex]
						...
						skill_blockpc_start (sd, MO_BODYRELOCATION, 2000);
					}
					break;
				case MO_TRIPLEATTACK:
					skillratio += 20*skill_lv;
		...
		if( sd )
		{
			ATK_ADDRATE(battle_SAD(sd, target, skill_num)); // Manage Skill Damage [Lilith]
...
#define MATK_ADD( a ) { ad.damage+= a; }
		...
		if(sd) {
			...
			MATK_ADDRATE(battle_SAD(sd, target, skill_num)); // Manage Skill Damage [Lilith]
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	if (sd)
	{
		...
		md.damage += (int64)md.damage * battle_SAD(sd, target, skill_num) / 100; // Manage Skill Damage [Lilith]
	}
